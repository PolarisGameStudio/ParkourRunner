...
namespace ParkourRunner.Scripts.Player.InvectorMods {
	public class ParkourThirdPersonController : vThirdPersonController {
		...
		public HeadDismember   HeadDismember;
		...
		public  bool  JetpackIsActive;

		public float BaseSpeed {
			set {
				_baseSpeed = value;
				SpeedMult  = _baseSpeed + _bonusBoostSpeed + _buttonSpeed;
			}
		}

		public float BonusBoostSpeed {
			set {
				_bonusBoostSpeed = value;
				SpeedMult        = _baseSpeed + _bonusBoostSpeed + _buttonSpeed;
			}
		}
		...
		private void ControllSpeed() {
			if (!_airSpeedFreeze && !JetpackIsActive) {
				...
				CurrRunSpeed = Utility.MapValue(_gm.GameSpeed, 1f, StaticConst.MaxGameSpeed, StaticConst.MinRunSpeed,
												StaticConst.MaxRunSpeed);
				CurrAnimSpeed = Utility.MapValue(_gm.GameSpeed, 1f, StaticConst.MaxGameSpeed, StaticConst.MinAnimSpeed,
												StaticConst.MaxAnimSpeed);
			}
		}
		...
		private void FixedUpdate() {
			...
			bool isBaseAction = isJumping || isRolling || isSliding || IsSlidingDown || IsSlidingTrolley ||
								this.IsPlayingAnimation;

			if (isGrounded                                   && !isBaseAction && !this.LoseBalance &&
				!_gm.IsLevelComplete                         &&
				_gm.gameState != GameManager.GameState.Pause && !TutorialMessage.IsShowMessage && !this.IsDoAction) {
			...
			if (IsSlidingTrolley) {
				float speed = 18f;
				transform.position = Vector3.MoveTowards(transform.position, TargetTransform.position + TrolleyOffset,
			...
			if (IsUsingHook) {
				transform.position = Vector3.MoveTowards(transform.position, TargetTransform.position + HookOffset,
														HookSpeed * Time.fixedDeltaTime);
				transform.rotation = TargetTransform.rotation;
				if (Vector3.Distance(TargetTransform.position + HookOffset, transform.position) <=
		...
		public override void Roll() {
			...
			bool rollConditions = (input != Vector2.zero || speed > 0.25f) && actionsRoll && isGrounded &&
		...
		public virtual void PlatformJump(float speed, float height) {
			...
			AudioManager.Instance.PlayUniqueSound(Gender.Kind == Gender.GenderKinds.Male
													? Sounds.JumpStrong
													: Sounds.JumpStrongFem);
		}
