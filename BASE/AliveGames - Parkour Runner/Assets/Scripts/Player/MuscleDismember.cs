...
namespace ParkourRunner.Scripts.Player {
	...
	public class MuscleDismember : MonoBehaviour {
		...
		public bool
			IsHandOrFeet = false; //При отчленении крайней части (ладонь, стопа) отваливается перыдущая (локоть, колено) потому что у нас нет такого количества моделек, да и это не нужно
		...
		public virtual void DismemberMuscleRecursive() {
			...
			foreach (var dism in newDismembers) {
				if (!dism.IsHandOrFeet && dism.NormalVersion.activeInHierarchy
				) //Является ли эта конечность последней (для них нет destroyedVersion) и не была ли она оторвана ещё раньше?
				{
