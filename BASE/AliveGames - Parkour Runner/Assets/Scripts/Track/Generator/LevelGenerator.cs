...
using Environment = ParkourRunnerEnvironment.Environment;
...
namespace ParkourRunner.Scripts.Track.Generator {
	public class LevelGenerator : MonoBehaviour {
		...
		[Header("Debug mode [key '1']")] [SerializeField]
		...
		private Block GetBlockFromList(List<Block> list) {
			...
			print($"generation block index: {index}");
			return list[index];
		}
		...
		public Vector3 GetRevivePosition() {
			var cb                                              = Instance.CenterBlock;
			// if (PhotonGameManager.IsMultiplayerAndConnected) cb = Instance.BlockPool[0];
			Vector3 newPos                                      = cb.transform.position;
			newPos.z -= Instance.BlockSize / 2f - 2f;
